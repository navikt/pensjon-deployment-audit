...
|------|--------------------------------------------------------|----------------------------------------|
| `audit_start_year` | `repositories.audit_start_year` alltid — også når den er `NULL` (betyr «ingen nedre grense» for hele repoet, ikke «ufastsatt») | `monitored_applications.audit_start_year` |
| `implicit_approval_mode` | `repositories.implicit_approval_mode` (aldri NULL) | `app_settings.implicit_approval` |
...
`audit_start_year` faller **ikke** tilbake til app-kolonnen når repoet er koblet, selv om repo-verdien er NULL — det ville undergravd garantien om at alle apper i repoet vurderes med identiske regler (en app som kobles til senere med en satt per-app-verdi ville ellers kunne avvike fra søsknene). `default_branch` faller derimot fortsatt tilbake per app, fordi NULL der kun betyr «ikke synket fra GitHub ennå», ikke et bevisst valg.

Fallbacken til app-nivå (når repoet ikke er koblet i det hele tatt) er nødvendig fordi `application_repositories.github_repo_id` fylles asynkront, og fordi `repositories.default_branch` først settes når den er hentet live fra GitHub. De gamle per-app-kolonnene beholdes derfor som fallback-lagring og skrives gjennom («write-through») ved hver repo-nivå-endring, slik at aggregerte spørringer som fortsatt leser dem gir samme resultat.

Endringer gjøres via `updateRepositorySettings` i [`repositories.server.ts`](../app/db/repositories.server.ts): den oppdaterer `repositories`, logger til `repo_config_audit_log`, speiler verdiene ned til alle aktive søsken-apper, og kjører baseline-rekalkulering (`pending_baseline`/`baseline`) for hele repo-scopet når `audit_start_year` endres. **Selve beslutningsalgoritmen i `verify.ts` er uendret** — kun kilden til `auditStartYear` og `implicitApprovalSettings` er flyttet.
